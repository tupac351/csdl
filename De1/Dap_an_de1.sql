...
-- Tạo Stored procedure DSNhanVienMuc2 (Location Transparency):
...
AS
    ...
    ELSE IF NOT EXISTS (SELECT * FROM PhongBanHN WHERE TenPB = @TenPB)
      AND NOT EXISTS (SELECT * FROM PhongBanSG WHERE TenPB = @TenPB)
        PRINT N'Không tìm thấy tên phòng ban!'
    ELSE IF EXISTS (SELECT * FROM PhongBanHN WHERE TenPB = @TenPB)
        SELECT nv.MaNV, nv.Ho, nv.Ten, nv.MaPB, pb.TenPB, pb.ChiNhanh
        FROM PhongBanHN pb
        JOIN NhanVienHN nv ON pb.MaPB = nv.MaPB
        WHERE pb.TenPB = @TenPB
    ELSE IF EXISTS (SELECT * FROM PhongBanSG WHERE TenPB = @TenPB)
        SELECT nv.MaNV, nv.Ho, nv.Ten, nv.MaPB, pb.TenPB, pb.ChiNhanh
        FROM PhongBanSG pb
        JOIN NhanVienSG nv ON pb.MaPB = nv.MaPB
        WHERE pb.TenPB = @TenPB
GO
-- Lưu ý: Các PB khác nhau có thể có TenPB trùng nhau → đúng ra phải dùng UNION.
-- Phiên bản đơn giản trên xử lý đúng khi TenPB là duy nhất trong từng phân mảnh.
